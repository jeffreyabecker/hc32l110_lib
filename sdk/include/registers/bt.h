...
#include <stdint.h>
  ...
  typedef enum
  {
    timer_function_timer = 0,
    timer_function_counter = 1,
  } timer_function_t;
  typedef enum
  {
    lp_timer_clock_source_peripheral_clock = 0,
    lp_timer_clock_source_xtl = 2,
    lp_timer_clock_source_rcl = 3
  } lp_timer_clock_source_t;
  ...
  typedef struct
  {
    ...
    volatile timer_function_t clock_source : 1;
  ...
  typedef struct
  {
    volatile uint8_t timer_running : 1;
    volatile basic_timer_mode_t mode : 1;
    volatile timer_function_t clock_source : 1;
    volatile uint8_t toggle_enabled : 1;
    volatile uint8_t clock_select : 2;
    uint32_t RESERVED6 : 1;
    volatile uint8_t write_allowed : 1;
    volatile uint8_t gate_enabled : 1;
    volatile uint8_t gate_polarity : 1;
    volatile uint8_t interrupt_enabled : 1;
  } hc32_lptimer_cr_flags_field_t;
  typedef struct
  {
    ...
    union
    {
      ...
      hc32_lptimer_cr_flags_field_t control_flags;
    };
    volatile uint32_t interrupt_flag;
    volatile uint32_t interrupt_clear;
  } hc32_lp_timer_register_t;
...
#define HC32_LPTIMER ((hc32_lp_timer_register_t *)LPTIMER_ADDRESS)
  typedef enum
  {
