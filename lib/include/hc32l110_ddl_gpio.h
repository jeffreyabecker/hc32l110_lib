...
#include "hc32l110_ddl_core.h"
    ...
    void gpio_configure(port_number_t port, gpio_port_config_t cfg);
    void gpio_configure_interrupt(port_number_t port, gpio_interrupt_config_t cfg, gpio_port_handler_t *handler);
...
#define gpio_disable_interrupt(port_number) (gpio_interrupt_config_t) port_number, NULL
    void gpio_configure_interrupt(port_number_t port, gpio_interrupt_config_t cfg, gpio_port_handler_t *handler)
    {
        __set_port_flag(port, HIE, cfg.interrupt_high);
        __set_port_flag(port, LIE, cfg.interrupt_low);
        __set_port_flag(port, RIE, cfg.interrupt_rising);
        __set_port_flag(port, FIE, cfg.interrupt_falling);
        __port_handlers[port] = handler;
    }
